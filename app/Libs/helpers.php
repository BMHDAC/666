...
function doImage($image_file): ?array
{
...
function doVideo($video_file, Dimension $dimension): ?array
{
    $ulid      = C_ULID::generate();
    $now       = $ulid->getDateTime();
    $directory = cdn_sc_path_files($now->format('Y/m/d'));
...
//    }
    ...
    $extension = $file->getClientOriginalExtension();
    $iid       = $ulid->toString();
    $cache[]   = "{$dimension->getWidth()}x{$dimension->getHeight()}";
    //save origin video
    $file->move($directory, "$iid.$extension");

    switch ($dimension->getRatio()->getValue()) {
        case 16 / 9:
            //1920x1080p
